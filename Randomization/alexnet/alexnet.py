...
save_model_path = str(SCRIPT_DIR / "alexnet_cifar10.pth")
...
def parse_args(args):
    parser = argparse.ArgumentParser(
        description="Script for running AlexNet with PyTorch"
    )

    parser.add_argument(
        "--dataset",
        help="cifar10 or cifar100, cifar10 is the default",
        default="cifar10",
    )
    parser.add_argument(
        "--learning-rate", help="learning rate", type=float, default=0.0001
    )
...
def main():
    ...
    x_train_resized = resize_images_to_224(
        x_train[:5000]
    ...
    for corruption_level in label_corruption_levels:
        ...
        y_train_corrupted = randomize_labels(
            y_train_subset.copy(), corruption_level, seed=25292
        )
        ...
        epochs_to_overfit = model.train(
            epochs=epochs, overfit_threshold=0.01, patience=patience, verbose=True
        )
        ...
        final_train_acc = (
            model.history["train_acc"][-1] if model.history["train_acc"] else 0
        )
        ...
        # Save model
        model.save_model(
            str(SCRIPT_DIR / f"alexnet_cifar10_corruption_{corruption_level:.1f}.pth")
        )
    ...
    plt.tight_layout()
    plt.savefig(str(SCRIPT_DIR / "alexnet_analysis.png"))
    plt.show()

    print("\nAnalysis complete!")
    print(f"Results saved to {SCRIPT_DIR / 'alexnet_analysis.png'}")
